import type { ExerciseInput, Measurement } from '@/session/reducer';
import { useSettings } from '@/settings/store';

export type Goal = 'hyrox' | 'half' | 'lift' | 'fit';
type Lift = { name: string; muscle: string; bodyweight?: boolean; measurement?: Measurement; sets: number; reps: number; rest?: number };
...
const L = (name: string, muscle: string, sets: number, reps: number, bodyweight = false, rest = 120, measurement?: Measurement): Lift => ({ name, muscle, sets, reps, bodyweight, rest, measurement: measurement ?? (bodyweight ? 'REPS_ONLY' : 'REPS_WEIGHT') });
export const WEEK: Record<Goal, PlannedSession[]> = {
  hyrox: [
    ...
    { day: 3, kind: 'lift', title: 'Push + carries', minutes: 60, lifts: [L('Barbell Bench Press', 'CHEST', 5, 5, false, 150), L('Barbell Overhead Press', 'SHOULDERS', 4, 6), L("Farmer's Walk", 'FOREARMS', 4, 40, false, 120, 'DISTANCE'), L('Push-Up', 'CHEST', 3, 15, true, 90), L('Plank', 'CORE', 3, 60, true, 60, 'DURATION')] },
    { day: 5, kind: 'run', title: 'Long easy 16 km', minutes: 95, detail: 'Zone 2' },
  ],
  half: [
    { day: 0, kind: 'lift', title: 'Strength for runners', minutes: 45, lifts: [L('Barbell Back Squat', 'QUADRICEPS', 4, 6, false, 150), L('Romanian Deadlift', 'HAMSTRINGS', 3, 8), L('Walking Lunge', 'QUADRICEPS', 3, 12), L('Plank', 'CORE', 3, 60, true, 60, 'DURATION')] },
  ...
  fit: [
    { day: 0, kind: 'lift', title: 'Full body A', minutes: 50, lifts: [L('Goblet Squat', 'QUADRICEPS', 3, 10), L('Dumbbell Bench Press', 'CHEST', 3, 10), L('Dumbbell Row', 'BACK', 3, 10), L('Plank', 'CORE', 3, 45, true, 60, 'DURATION')] },
...
export function toSession(s: PlannedSession): ExerciseInput[] {
  return (s.lifts ?? []).map((l) => ({
    exerciseId: localId(l.name), name: l.name, muscle: l.muscle, bodyweight: l.measurement !== 'REPS_WEIGHT', measurement: l.measurement, restSeconds: l.rest ?? useSettings.getState().restSeconds,
